...
HTML_OUT = 'rdkit_maccs_report.html'
PLOTS_DIR = 'plots'
...
# Global plot styling (Gridlines enabled)
...
def main():
    ...
    for sm in tqdm(df_raw['SMILES'], desc="Calculating Descriptors & Fingerprints"):
        ...
        if mol:
            try:
                ...
                if atom_c and bond_c and polar_c and ring_c:
                    custom_vals = list(atom_c.values()) + list(bond_c.values()) + list(polar_c.values()) + list(ring_c.values())
                    results.append(rdkit_vals + maccs_vals + custom_vals)
                    valid_smiles.append(sm)
            except Exception as e:
                failed_mols.append({'SMILES': sm, 'Reason': f"Featurization Error: {str(e)}", 'Missing_Columns': 'N/A'})
                continue
        else:
            failed_mols.append({'SMILES': sm, 'Reason': "Invalid SMILES", 'Missing_Columns': 'N/A'})

    dummy = Chem.MolFromSmiles('C1CCCCC1')
    atom_names = list(calculate_atom_counts(dummy).keys())
    bond_names = list(calculate_bond_counts(dummy).keys())
    polar_names = list(calculate_polar_bond_counts(dummy).keys())
    ring_names = list(calculate_ring_counts(dummy).keys())
    
    custom_names = atom_names + bond_names + polar_names + ring_names
    custom_no_atoms = bond_names + polar_names + ring_names
    all_col_names = rdkit_names + maccs_names + custom_names
    ...
    removed_mols_log = pd.DataFrame(columns=['SMILES', 'Reason', 'Missing_Columns'])
    if failed_mols:
        removed_mols_log = pd.concat([removed_mols_log, pd.DataFrame(failed_mols)], ignore_index=True)
    ...
    # 1. Column NaNs (>10% threshold)
    nan_threshold = 0.10 * len(df_merged)
    nan_counts = df_merged.isna().sum()
    nan_cols = [c for c in nan_counts[nan_counts > nan_threshold].index if c not in ['SMILES', 'LogS_Median']]
    for col in nan_cols: removed_features_log.append({'Feature': col, 'Reason': f'NaN threshold (>10%)'})
    df_merged.drop(columns=nan_cols, inplace=True)
    
    # 2. Row NaNs (Track exact missing columns)
    nan_rows_mask = df_merged.isna().any(axis=1)
    nan_mols_df = df_merged[nan_rows_mask].copy()

    if not nan_mols_df.empty:
        missing_cols = nan_mols_df.apply(lambda row: ", ".join(row.index[row.isna()]), axis=1)
        nan_mols_log = pd.DataFrame({
            'SMILES': nan_mols_df['SMILES'],
            'Reason': 'Dropped due to NaN values',
            'Missing_Columns': missing_cols
        })
        removed_mols_log = pd.concat([removed_mols_log, nan_mols_log], ignore_index=True)
    ...
    for col in zero_var_cols: removed_features_log.append({'Feature': col, 'Reason': 'Zero Variance'})
    ...
    for col in feature_cols:
        if (df_merged[col] != 0).sum() <= 5:
            low_freq_cols.append(col)
            removed_features_log.append({'Feature': col, 'Reason': 'Low Frequency (<= 5)'})
    ...
    surviving_maccs = [f for f in features if f.startswith('MACCS_')]
    surviving_custom_with_atoms = [f for f in features if f in custom_names]
    surviving_custom_no_atoms = [f for f in features if f in custom_no_atoms]

    # --- OPTUNA (Scaling inside folds to prevent leakage) ---
    ...
    def objective(trial):
        ...
        for t_idx, v_idx in kf.split(X_train_raw):
            ...
            X_fold_valid_sc = fold_scaler.transform(X_fold_valid)
    ...
    b64_shap_all = save_shap_subset(features, "shap_all.png")
    b64_shap_maccs = save_shap_subset(surviving_maccs, "shap_maccs_only.png")

    # 10 MACCS Keys (Importance Bar Chart)
    importances = model.feature_importances_
    maccs_idx = [features.index(f) for f in surviving_maccs]
    maccs_importances = importances[maccs_idx]
    
    top_10_local_idx = np.argsort(maccs_importances)[::-1][:10]
    top_10_scores = maccs_importances[top_10_local_idx]
    top_10_names = [surviving_maccs[i] for i in top_10_local_idx]
    
    maccs_labels = []
    for name in top_10_names:
        bit_num = int(name.split('_')[1])
        if bit_num in MACCSkeys.smartsPatts:
            smarts_pattern = MACCSkeys.smartsPatts[bit_num][0]
            maccs_labels.append(f"Bit {bit_num}: {smarts_pattern}")
        else:
            maccs_labels.append(f"Bit {bit_num}: Unknown Pattern")
            
    fig_top10 = plt.figure(figsize=(10, 6))
    sns.barplot(x=top_10_scores, y=maccs_labels, palette='viridis')
    plt.title("")
    plt.xlabel("XGBoost Feature Importance (Gain)")
    plt.tight_layout()
    b64_top10_maccs = save_and_get_b64(fig_top10, "top_10_maccs.png")

    # Correlation to LogS (With & Without Atoms)
    ...
    b64_corr_no_atoms = plot_logs_corr(surviving_custom_no_atoms, "corr_logs_no_atoms.png")
    b64_corr_with_atoms = plot_logs_corr(surviving_custom_with_atoms, "corr_logs_with_atoms.png")

    # Heatmap Matrix
    valid_rdkit = [c for c in RDKIT_TARGETS if c in train_df.columns]
    corr_matrix = train_df[surviving_custom_with_atoms + valid_rdkit].corr().loc[surviving_custom_with_atoms, valid_rdkit]
    
    fig_corr = plt.figure(figsize=(12, max(8, len(surviving_custom_with_atoms)*0.4)))
    ...
    <h3>MACCS Keys Only</h3>
    <img src="data:image/png;base64,{b64_shap_maccs}">
    
    <h2 class="section-title">4. Top 10 MACCS Keys (Feature Importance)</h2>
    <img src="data:image/png;base64,{b64_top10_maccs}">

    <h2 class="section-title">5. LogS Correlation</h2>
    <h3>Custom Features (Without Atoms)</h3>
    <img src="data:image/png;base64,{b64_corr_no_atoms}">
    <h3>Custom Features (With Atoms)</h3>
